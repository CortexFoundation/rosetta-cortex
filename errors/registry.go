...
func (r errorRegistry) add(err *Error) {
	...
	defer r.mu.Unlock()
	if r.sealed {
		_, _ = fmt.Fprintln(os.Stderr, "[CORTEX] WARNING: attempts to register errors after seal will be ignored")
	}
	if _, ok := r.errors[err.rosErr.Code]; ok {
		_, _ = fmt.Fprintln(os.Stderr, "[CORTEX] WARNING: attempts to register an already registered error will be ignored, code: ", err.rosErr.Code)
	}
